...
authorizedAxiosInstance.interceptors.response.use(
  ...
  (error) => {
    ...
    //TH1: Nếu nhận mã 401 hoặc 410 từ BE, thử refresh token trước
    const originalRequests = error.config;
    
    if ((error.response?.status === 401 || error.response?.status === 410) && !originalRequests._retry) {
      // Không retry nếu đang gọi API logout hoặc refresh-token
      if (originalRequests?.url?.includes('/users/logout') || originalRequests?.url?.includes('/users/refresh-token')) {
      ...
      // Kiểm tra xem nếu chưa có refreshTokenPromise thì thực hiện gọi api refresh_token đồng thời gán vào refreshTokenPromise
    ...
    //Dùng toastify để hiển thị bất kể mọi mã lỗi lên màn hình - Ngoại trừ 401 và 410 khi đang retry
