...
const ResultsTray = ({ flashcards, onClear, onSaveAll, onRemoveCard, saving }) => {
    ...
    <div className="flex flex-col min-h-full bg-white border-2 border-b-[4px] rounded-3xl overflow-hidden relative transition-all"
      ...
      <header className="px-5 py-4 bg-white border-b-2 flex items-center justify-between sticky top-0 z-20"
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-[15px] font-black leading-none tracking-tight uppercase" style={{ color: t.text }}>
              Kết quả
            </h3>
            <div className="flex items-center gap-1.5 mt-1.5">
              <span className="w-2 h-2 rounded-full" style={{ backgroundColor: isEmpty ? t.gray : t.green }} />
              <p className="text-[11px] font-bold uppercase tracking-widest" style={{ color: t.sub }}>
            ...
            <div style={{ position: 'relative', width: 72, height: 72, marginBottom: 20 }}>
              {[2, 1, 0].map((i) => (
                <div key={i} style={{
                  ...
                  borderBottom: `3px solid ${t.grayDark}`,
                  background: i === 0 ? '#fff' : i === 1 ? '#F7F7F7' : '#F0F0F0',
                  top: i * 8, left: i * 6,
